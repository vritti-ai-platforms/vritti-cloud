...
export class AppService {
  ...
  // Creates a new app; throws ConflictException on duplicate code within the same version + business
  async create(dto: CreateAppDto): Promise<CreateResponseDto<AppDto>> {
    const existing = await this.appRepository.findByVersionBusinessCode(dto.versionId, dto.businessId, dto.code);
    if (existing) {
      throw new ConflictException({
        label: 'Code Already Exists',
        detail: 'An app with this code already exists for this business. Please choose a different code.',
  ...
  // Returns apps for a version + business with counts, applying server-stored filter/sort/search/pagination state
  async findForTable(userId: string, versionId: string, businessId: string): Promise<AppTableResponseDto> {
    const { state, activeViewId } = await this.dataTableStateService.getCurrentState(
      userId,
      `apps-${versionId}-${businessId}`,
    );
    const scopeFilters: FilterCondition[] = [
      { field: 'versionId', operator: 'equals', value: versionId },
  ...
  async findById(id: string): Promise<AppDto> {
    ...
    return AppDto.from(row, row.featureCount);
  }
  ...
  async update(id: string, dto: UpdateAppDto): Promise<SuccessResponseDto> {
    ...
    if (dto.code && dto.code !== existing.code) {
      const existingCode = await this.appRepository.findByVersionBusinessCode(
        existing.versionId,
        existing.businessId,
        dto.code,
      );
  ...
  async importFromFile(buffer: Buffer, versionId: string, businessId: string): Promise<ImportResponseDto> {
    const rows = parseSpreadsheet(buffer);
    const result = await validateImportRows(rows, CreateAppDto, { versionId, businessId });

    for (const row of result.rows) {
      if (!row.valid) continue;
      const existing = await this.appRepository.findByVersionBusinessCode(versionId, businessId, row.data.code);
      if (existing) {
        row.valid = false;
    ...
    this.logger.log(
      `Imported apps for version ${versionId}, business ${businessId}: ${created} created, ${updated} updated`,
    );
    return { success: true, message: 'Import complete.', created, updated };
  }

  // Exports all apps for a version + business as an Excel buffer
  async exportToBuffer(versionId: string, businessId: string, format: ExportFormat = 'xlsx'): Promise<Buffer> {
    const allApps = await this.appRepository.findAllByVersionAndBusiness(versionId, businessId);
    ...
    this.logger.log(`Exported ${rows.length} app(s) for version ${versionId}, business ${businessId}`);
