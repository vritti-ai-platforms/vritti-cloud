import { AppFeatureService } from '@domain/version/business/app/app-feature/services/app-feature.service';
import { Controller, Delete, Get, HttpCode, HttpStatus, Logger, Param, Post, Put } from '@nestjs/common';
...
@Controller('versions/:versionId/businesses/:businessId/apps/:appId/features')
export class AppFeatureController {
  ...
  @ApiAssignFeature()
  assignFeature(@Param('appId') appId: string, @Param('featureId') featureId: string): Promise<SuccessResponseDto> {
    this.logger.log(`POST /admin-api/apps/${appId}/features/${featureId}`);
    return this.appFeatureService.assignFeature(appId, featureId);
  }
  ...
  @ApiToggleAppFeature()
  toggleFeature(@Param('appId') appId: string, @Param('featureId') featureId: string): Promise<SuccessResponseDto> {
    this.logger.log(`PUT /admin-api/apps/${appId}/features/${featureId}`);
    return this.appFeatureService.toggleFeature(appId, featureId);
  }
