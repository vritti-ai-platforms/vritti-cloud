...
import { ApiDeleteAccount, ApiGetProfile, ApiUpdateProfile } from '../docs/profile.docs';
...
export class ProfileController {
  ...
  async getProfile(@UserId() userId: string): Promise<UserDto> {
    this.logger.log(`GET /account/profile - Fetching profile for user: ${userId}`);
  ...
  async updateProfile(@UserId() userId: string, @Body() updateUserDto: UpdateUserDto): Promise<UserDto> {
    this.logger.log(`PUT /account/profile - Updating profile for user: ${userId}`);
  ...
  async deleteAccount(@UserId() userId: string): Promise<{ message: string }> {
    this.logger.log(`DELETE /account/account - Deleting account for user: ${userId}`);
