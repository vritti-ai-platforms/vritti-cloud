...
export const AppsTab = ({ businessId, roleId }: { businessId: string; roleId: string }) => {
  const queryClient = useQueryClient();
  const { versionId } = useVersionContext();
  const { data: response, isLoading } = useRoleTemplateAppsTable(versionId, businessId, roleId);
  ...
  function handleToggle(row: RoleTemplateAppTableRow) {
    if (row.isAssigned) {
      removeMutation.mutate(
        { versionId, businessId, roleTemplateId: roleId, appId: row.appId },
