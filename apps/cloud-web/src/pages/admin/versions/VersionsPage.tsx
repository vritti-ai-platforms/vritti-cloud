...
export const VersionsPage = () => {
  ...
  const { data: response, isLoading } = useVersionsTable();
  ...
  const { table } = useDataTable({
    ...
    onStatePush: () => queryClient.invalidateQueries({ queryKey: VERSIONS_TABLE_KEY }),
  });

  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Versions" description="Manage platform versions and their configuration" />
...
function getColumns({ onView }: ColumnActions): ColumnDef<Version, unknown>[] {
  return [
    {
      accessorKey: 'version',
      header: 'Version',
