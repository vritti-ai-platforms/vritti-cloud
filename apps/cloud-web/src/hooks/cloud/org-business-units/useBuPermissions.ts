...
export type BuUnlockPayload = { memberships: MatrixMembership[] };
...
// Fetches the BU matrix — the plan ceiling with the BU's current memberships nested under each app
export function useBuPermissionMatrix(orgId: string, buId: string) {
  return useQuery<{ apps: MatrixApp[] }, AxiosError>({
