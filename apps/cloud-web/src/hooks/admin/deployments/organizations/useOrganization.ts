...
import { getOrganization } from '@/services/admin/deployments/organizations.service';

// Fetches a single organization by ID — suspends until data is ready
export function useOrganization(deploymentId: string, id: string) {
  return useSuspenseQuery<AdminOrganizationDetail, AxiosError>({
    queryKey: ['admin', 'organizations', id],
    queryFn: () => getOrganization(deploymentId, id),
  });
}
