...
export function getFeaturePermissions(versionId: string, featureId: string): Promise<{ types: string[] }> {
  return axios
    .get<{ types: string[] }>(`admin-api/versions/${versionId}/features/${featureId}/permissions`)
    .then((r) => r.data);
}
...
export function setFeaturePermissions(versionId: string, featureId: string, types: string[]): Promise<SuccessResponse> {
  return axios
    .put<SuccessResponse>(`admin-api/versions/${versionId}/features/${featureId}/permissions`, { types })
    .then((r) => r.data);
}
