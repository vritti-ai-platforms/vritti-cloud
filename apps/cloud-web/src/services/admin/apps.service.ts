...
export function getApps(versionId: string): Promise<AppsTableResponse> {
  return axios.get<AppsTableResponse>(`admin-api/app-versions/${versionId}/apps/table`).then((r) => r.data);
}

// Fetches a single app with counts
export function getApp(versionId: string, id: string): Promise<App> {
  return axios.get<App>(`admin-api/app-versions/${versionId}/apps/${id}`).then((r) => r.data);
}
...
export function updateApp(versionId: string, { id, data }: { id: string; data: UpdateAppData }): Promise<SuccessResponse> {
  return axios.patch<SuccessResponse>(`admin-api/app-versions/${versionId}/apps/${id}`, data).then((r) => r.data);
}

// Deletes an app by ID
export function deleteApp(versionId: string, id: string): Promise<void> {
  return axios.delete(`admin-api/app-versions/${versionId}/apps/${id}`).then(() => undefined);
}
...
export function getAppFeaturesTable(versionId: string, appId: string): Promise<AppFeaturesTableResponse> {
  return axios.get<AppFeaturesTableResponse>(`admin-api/app-versions/${versionId}/apps/${appId}/features/table`).then((r) => r.data);
}

// Fetches features assigned to an app
export function getAppFeatures(versionId: string, appId: string): Promise<AppFeature[]> {
  return axios.get<AppFeature[]>(`admin-api/app-versions/${versionId}/apps/${appId}/features`).then((r) => r.data);
}
...
export function assignAppFeatures({
  ...
  return axios.post<SuccessResponse>(`admin-api/app-versions/${versionId}/apps/${appId}/features`, data).then((r) => r.data);
}
...
export function removeAppFeature({ versionId, appId, featureId }: { versionId: string; appId: string; featureId: string }): Promise<void> {
  return axios.delete(`admin-api/app-versions/${versionId}/apps/${appId}/features/${featureId}`).then(() => undefined);
}

// Fetches prices for an app
export function getAppPrices(versionId: string, appId: string): Promise<AppPrice[]> {
  return axios.get<AppPrice[]>(`admin-api/app-versions/${versionId}/apps/${appId}/prices`).then((r) => r.data);
}
...
export function updateAppPrice({
  ...
  return axios.patch<SuccessResponse>(`admin-api/app-versions/${versionId}/apps/${appId}/prices/${priceId}`, data).then((r) => r.data);
}
...
export function removeAppPrice({ versionId, appId, priceId }: { versionId: string; appId: string; priceId: string }): Promise<void> {
  return axios.delete(`admin-api/app-versions/${versionId}/apps/${appId}/prices/${priceId}`).then(() => undefined);
}
